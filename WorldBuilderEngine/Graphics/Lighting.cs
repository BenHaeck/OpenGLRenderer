...
vec3 getLight (vec3 pos, vec3 normal) {
	...
	for (int i = 0; i < lightNumber; i++) {
		...
		distSqr3 *= distSqr3;
		float dist = distSqr3.x + distSqr3.y + distSqr3.z;
		float dirMult = dot(normalize(lightPos-pos),normal);
		if (dist > radius * radius || radius <= 0.01 || dirMult < 0.00)
			continue;

		dist = sqrt(dist);

		lightChange += vec3(lightVals[si+4],lightVals[si+5],lightVals[si+6]) *
		dirMult * (1 - dist / radius);
	...
	public static void Add (float posX, float posY, float posZ, float radius, float r, float g, float b) {
		if (numberOfLights >= LIGHT_NUMBER_MAX || Renderer.ShouldBeCulled(posX, posY, posZ, radius))
			return;
